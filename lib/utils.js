...
const nodeUrl = () => {
    return JSON
        .parse((0, fs_1.readFileSync)((0, path_1.join)(process.cwd(), 'muffin.config.json'), { encoding: 'utf-8' })).networks[process.argv[0] !== 'mocha' ? process.argv[2] : 'local'].url;
};
exports.nodeUrl = nodeUrl;
const giver = () => {
    return JSON
        .parse((0, fs_1.readFileSync)((0, path_1.join)(process.cwd(), 'muffin.config.json'), { encoding: 'utf-8' })).networks[process.argv[0] !== 'mocha' ? process.argv[2] : 'local'].giver;
};
exports.giver = giver;
const keys = () => {
    return JSON
        .parse((0, fs_1.readFileSync)((0, path_1.join)(process.cwd(), 'muffin.config.json'), { encoding: 'utf-8' })).networks[process.argv[0] !== 'mocha' ? process.argv[2] : 'local'].keys;
};
exports.keys = keys;
