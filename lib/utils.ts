...
export const nodeUrl = (): string => {
  return JSON
    ...
    ).networks[process.argv[1].includes('mocha') ? 'local' : process.argv[2]].url;
}
...
export const giver = (): GiverOptions => {
  return JSON
    ...
    ).networks[process.argv[1].includes('mocha') ? 'local' : process.argv[2]].giver;
}
...
export const keys = (): KeysOptions => {
  return JSON
    ...
    ).networks[process.argv[1].includes('mocha') ? 'local' : process.argv[2]].keys;
}
