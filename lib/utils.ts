...
export const nodeUrl = (): string => {
  return JSON
    ...
    ).networks[process.argv[0] !== 'mocha' ? process.argv[2] : 'local'].url;
}
...
export const giver = (): GiverOptions => {
  return JSON
    ...
    ).networks[process.argv[0] !== 'mocha' ? process.argv[2] : 'local'].giver;
}
...
export const keys = (): KeysOptions => {
  return JSON
    ...
    ).networks[process.argv[0] !== 'mocha' ? process.argv[2] : 'local'].keys;
}
