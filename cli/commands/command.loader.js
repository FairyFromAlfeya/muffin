...
class CommandLoader {
    static load(program) {
        new init_command_1.InitCommand(new index_1.InitAction()).load(program);
        new build_command_1.BuildCommand(new index_1.BuildAction()).load(program);
    ...
    static handleInvalidCommand(program) {
        program.on('command:*', () => {
            console.error(`\n${index_2.ERROR_PREFIX} Invalid command: ${chalk.red('%s')}`, program.args.join(' '));
